...
public class TransformNR {
    ...
    public static String getMatrixString(Matrix3D matrix) {
        ...
        for (int i = 0; i < across; i++) {
            ...
            for (int j = 0; j < down; j++) {
                if (m[i][j] < 0) s.append(new DecimalFormat("000.00").format(m[i][j]));
                else s.append(new DecimalFormat("0000.00").format(m[i][j]));
                if (j < down - 1) s.append(",");
                s.append("\t");
            }
            s.append(" }");
            if (i < across - 1) s.append(",");
            s.append("\n");
        }
        return s + "}\n";
    }
    ...
    public TransformNR setX(double translation) {
        assert !Double.isNaN(translation) : "Value can not be NaN";
        x = translation;
        return this;
    }
    ...
    public final boolean equals(Object o) {
        if (!(o instanceof TransformNR that)) return false;

        return Double.compare(x, that.x) == 0 && Double.compare(y, that.y) == 0 && Double.compare(z, that.z) == 0 && rotation.equals(that.rotation);
    }
